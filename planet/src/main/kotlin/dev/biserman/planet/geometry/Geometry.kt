...
fun (Double).adjustRange(oldRange: ClosedRange<Double>, newRange: ClosedRange<Double>): Double = (this - oldRange.start) / (oldRange.endInclusive - oldRange.start) * (newRange.endInclusive - newRange.start) + newRange.start

fun (Float).adjustRange(oldRange: ClosedRange<Float>, newRange: ClosedRange<Float>): Float = (this - oldRange.start) / (oldRange.endInclusive - oldRange.start) * (newRange.endInclusive - newRange.start) + newRange.start
...
fun (Iterable<Pair<Vector3, Double>>).weightedAverageInverse(reference: Vector3, maxDistance: Double) = this.weightedAverage(reference) { 1 - (it.distanceTo(reference) / maxDistance) }

fun (Iterable<Pair<Vector3, Double>>).weightedAverageInverse(reference: Vector3) = this.weightedAverage(reference) { 1 / it.distanceTo(reference) }

fun (Iterable<Pair<Vector3, Double>>).weightedAverage(reference: Vector3) = this.weightedAverage(reference) { point -> reference.distanceTo(point) }
...
fun sigmoid(x: Float, xScalar: Float = -1.0f, xOffset: Float = 0.0f) = 1f / (1 + E.toFloat().pow(xScalar * (x + xOffset)))

fun intersectRaySphere(rayOrigin: Vector3, rayDir: Vector3, sphereCenter: Vector3, sphereRadius: Double): Double? {
...
fun (Double).scaleAndCoerceIn(expectedRange: ClosedRange<Double>, newRange: ClosedRange<Double>) = this.adjustRange(expectedRange, newRange).coerceIn(newRange)
...
fun longitudeDistanceDegrees(first: Double, second: Number): Double = ((first - second.toDouble() + 540.0) % 360.0 - 180.0).absoluteValue

fun bestOceanCorridorDateLineDegrees(meridianLandCoverage: IntArray, landMasses: Iterable<Pair<Double, Double>>): Double {
