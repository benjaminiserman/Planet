...
data class StonePlacement(val type: StonePlacementType, val specialOptions: List<SpecialOption>, val defaultOption: Stone) {
    data class SpecialOption(val condition: StonePlacementCondition, val stone: Stone)
}

data class StoneColumn(@field:JsonIdentityReference(alwaysAsId = true) var surface: Stone, @field:JsonIdentityReference(alwaysAsId = true) var middle: Stone, @field:JsonIdentityReference(alwaysAsId = true) var deep: Stone) {
