...
object ClimateSimulation {
    ...
    private data class MoistureRoute(val neighborId: Int, val moistureFraction: Double, val upslopePrecipitation: Double)
    ...
    fun estimateMonth(planet: Planet, daysPassed: Number): String = when (((daysPassed.toDouble() % yearLength) / (365.242 / 12.0)).toInt() + 1) {
        1 -> "Jan"
        2 -> "Feb"
        3 -> "Mar"
        4 -> "Apr"
...
//        updatePlanetClimate(planet)
    ...
    fun (Planet).calculateItcz(): Path<PlanetTile> {
        fun costFn(_1: PlanetTile, tile: PlanetTile): Double = max(
            0.0001,
