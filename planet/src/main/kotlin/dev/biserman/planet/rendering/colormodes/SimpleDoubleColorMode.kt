...
class SimpleDoubleColorMode(planetRenderer: PlanetRenderer, override val name: String, val colorFn: (Double?) -> Color = defaultColorFn, override val categories: List<String>, val getFn: (PlanetTile) -> Double?) : PlanetColorMode(planetRenderer) {
...
//            colorFn(validTilesValues.average())
    ...
    companion object {
        ...
        fun redOutsideRange(range: ClosedRange<Double>, colorFn: (Double) -> Color = defaultColorFn) = redWhenNull { if (it in range) colorFn(it) else Color.red }
