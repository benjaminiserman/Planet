...
class TectonicPlateBoundaryRenderer(parent: Node, val lift: Double, override val categories: List<String> = listOf()) : DebugRenderer<Planet>(parent) {
    override val name = "tectonic_plate_boundary"

    override fun generateMeshes(input: Planet): List<MeshData> = input.tectonicPlates.withIndex().map { (index, plate) ->
